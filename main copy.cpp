...
int main() {
    ...
    empleadoCargado.mostrar();
     ...
     // Cargar datos desde archivos para verificar persistencia
    ifstream archivoClientesIn("clientes.txt"), archivoEmpleadosIn("empleados.txt"), archivoOrdenesIn("ordenes.txt");
     list<Articulo*> articulos = { articulo1, articulo2, articulo3 };  // Artículos para asociar en detalles de orden
    Orden ordenCargada("ORD001", &clienteCargado, &empleadoCargado);
    ordenCargada.cargar(archivoOrdenesIn, &clienteCargado, &empleadoCargado, articulos);
